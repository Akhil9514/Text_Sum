...
class Batch(object):
    ...
    def __init__(self, data=None, device=None,  is_test=False):
        ...
        if data is not None:
            ...
            mask = 1 - (src == 0).to(torch.float32)

            clss = torch.tensor(self._pad(pre_clss, -1))
            mask_cls = 1 - (clss == -1).to(torch.float32)
            clss[clss == -1] = 0
...
def load_files(directory_path,corpus_type, file_extension='*'):
...
def load_dataset(args, corpus_type, shuffle):
    ...
    def _lazy_dataset_loader(pt_file, corpus_type):
        ...
        dataset = dataset[0]
        print("lazy loader dataset 🔥",dataset)
    ...
    directory_path = args.bert_data_path
    pts = load_files(directory_path, corpus_type)
    if pts:
        if (shuffle):
            random.shuffle(pts)
